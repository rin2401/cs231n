...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    exp_scores = np.exp(scores - np.max(scores, axis=1, keepdims=True)) # N x C
    probs = exp_scores / np.sum(exp_scores, axis = 1, keepdims=True) # N x C
    L_i = - np.log(probs[range(N), y]) # N x 1
    data_loss = np.sum(L_i) / N 
    reg_log = reg * (np.sum(W1 * W1) + np.sum(W2 * W2)) 
    loss = data_loss + reg_log
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
